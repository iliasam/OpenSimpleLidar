...
{
  uint16_t tmp_cap_number = cap_number;//для защиты от возможного изменения переменной
  uint8_t  tmp_res_buf_num = res_buf_num;
  uint16_t tmp_centroid = 0;
  
  if ((meas_status == PHASE_DONE) && (proc_done_flag == 0))//изображения последнего градуса захвачены и обработка еще не выполнялась
  {
    //switch_led(1);
    //tmp_centroid = 3200;
    ...
    if (tmp_res_buf_num == 0)
      res_buf0[tmp_cap_number + PACKET_OFFSET] = (uint16_t)tmp_centroid; 
    else
      res_buf1[tmp_cap_number + PACKET_OFFSET] = (uint16_t)tmp_centroid;
    
    
    proc_done_flag = 1;//защита от повторного запуска
    //switch_led(0);
