...
//Forced stop image capture
void stop_hardware_capture(void)
{
  TIM1->CR1 &= (uint16_t)(~((uint16_t)TIM_CR1_CEN));//Stop ADC timer
...
void adc_dma_init(void)
{
  ...
  DMA_InitStructure.DMA_MemoryBaseAddr     = (uint32_t)&data_adc_laser0[0];
...
{ 
  ...
  ADC_StartOfConversion(ADC1);
  TIM_SetCounter(TIM1,0);
  adc_dma_config((uint16_t*)pointer);
